...
|----------------------+----------------------------|
| ChatGPT              | [[https://platform.openai.com/account/api-keys][API key]] or ChatGPT Plus/Pro login |
...
If you want to use ChatGPT Codex access instead of an OpenAI API key,
register the OAuth backend explicitly:
...
This uses OpenAI's device-code login flow:

1. =gptel-openai-chatgpt-login= opens =https://auth.openai.com/codex/device=
   and copies a one-time code to the clipboard.
2. Enter the code in your browser and approve access for your ChatGPT
   account.
3. Return to Emacs and press =RET= to let gptel finish the token
   exchange.

You do /not/ need to create or copy any token manually from the OpenAI
web UI for this flow.

If your browser opens a callback page after approval, or prompts for
credentials for that callback, you can ignore it and close the tab.
gptel completes the login back in Emacs and stores the OAuth token in
=gptel-openai-chatgpt-token-file=.

Note: this ChatGPT Codex backend is streaming-only.  It always uses
streaming responses, so setting =gptel-stream= to =nil= is not
